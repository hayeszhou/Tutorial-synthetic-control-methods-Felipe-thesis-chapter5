---
title: "Assesing the significance of cct policies on hpomicide rates using synthetic analysis - Colombian municipalities in the pacific region"
...
gen_ori <- read_excel("data/PANEL_CARACTERISTICAS_GENERALES(2019).xlsx")
gen<- gen_ori %>% 
  filter(ano==2005) %>% 
  select(1:5,ipm_asisescu_pob)
gen
```
...
```{r}
library(readxl)
cct2 <- read_excel("C:/Users/felip/OneDrive/Desktop/Meidai/THESIS/M2021-Felipe-SDGs-Crime-Colombia/data/rawData/CCTs/raw_data_andes_cct2005_2010.xlsx", 
    skip = 1)
cct2
...
In some municipalities more children received the help in the first year 2006 than in 2010, those municipalities are removed fora preliminary analysis.
...
# Fithered series of homicide rates 
...
## creating a dataframes for munimiciaplities below different population thresholds
...
# creating the dataset of crime determinants for the municipaliteis in the pacific region
...
#  Finding the municiaplities in the contron and treatment groups 

## categorical variable municipalities below and above the coverage threshold

```{r}
# control group coverage belo 30%
low=30
...
new_det<-right_join(new_det, cluster, by="codmpio")
new_det <- new_det %>% select(-eb_mr)
...
nmr <- nmr %>% filter(codmpio %in% new_det$codmpio )

new_syn<- full_join(nmr, new_det, by=c("codmpio", "year")) %>% 
...
Some extra municipalities  (including islands) must be removed


#```{r}
new_syn %>% 
  group_by(codmpio) %>% 
  summarise(a=n_distinct(year)) %>% 
  arrange(a)
 new_syn %>% 
   filter(!codmpio %in% c(23685,97777,27086,88001,88564,0.1)) %>% 
   group_by(codmpio) %>% 
  summarise(a=n_distinct(year)) %>% 
  arrange(a)
 
  new_syn <- new_syn %>% 
   filter(!codmpio %in% c(23685,27086, 97777,88001,88564,0.1))
...
#new_syn %>%  filter(codmpio %in% control) %>%
 # filter(year %in% c(2004,2007,2010))
```
...
#  tryCatch is used so that the for loop continues even if there is an error in of the steps
   tryCatch({
...
If the for loop was carried out xx=63

The mr_gap_pacific_states dataframe has the gap (real - synthetic) for each of the 63 regions
...
In case the dataframe mr_gap_pacific_states is saved

```{r}
mr_gap_pacific_states

#loc_file<- paste("output/gap_cct_synthetic_pacific_states",pop, as.character(low), as.character(high),".csv",sep="_")

#write.csv(mr_gap_pacific_states, loc_file)

```

It can be seen that Loss V is the mean predicted square error before the treatment 
...
# calculation of the post treatment mean square predicted error
...
post.pre <- inner_join( post_mspe, table.loss, by=c("municipality"="mun")) %>% 
  ...
  mutate(post_pre= sqrt(post_mspe_gap)/sqrt(Loss.V))
post.pre
...
set.seed(2019)
post.pre %>% 
ggplot( aes(x=high_cct, y=(post_pre), color=high_cct)) +
  coord_flip()+
...
post.pre.high<- post.pre   %>% 
  filter(high_cct=="high") %>% select(post_pre)

post.pre.low <-post.pre %>% 
  filter(high_cct=="low") %>% select(post_pre)
...
filen<- paste("output/50_50/", namenn, sep="")
#filen
#pdf(filen, width = 7, height = 7)
# 2. Create the plot
#path.plot(synth.res = synth.out, dataprep.res = dataprep.out,
#Ylab = "Empirical Bayes homicide rate (per 10000)", Xlab = "year", Legend = c("high CCT coverage municipality",
#"synthetic control"), Legend.position = "topright", tr.intake = 2011)
# 3. Close the file
#dev.off()


#gaps.plot(synth.res = synth.out, dataprep.res = dataprep.out, Ylab = "gap in homicides", Xlab = "year", Ylim = c(-10, 10), Main = NA)
